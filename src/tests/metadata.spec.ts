...
describe('ArchiveOfOurOwn::fetchWorksMetadata', () => {
  it('fetchWorksMetadata::worksName', () => {
    ...
    it('fetchWorksMetadata::authorName', () => {
      let fic = new ArchiveOfOurOwn();
      return fic.fetchWorksMetadata(worksUrl, params).then((response: any)=> {
          const assertVal = (response.authorName == expectedResponse.authorName);
          assert.isTrue(assertVal);
        });
      })

      it('fetchWorksMetadata::worksFandom', () => {
        let fic = new ArchiveOfOurOwn();
        return fic.fetchWorksMetadata(worksUrl, params).then((response: any)=> {
            const assertVal = (response.worksFandom == expectedResponse.worksFandom);
            assert.isTrue(assertVal);
          });
        })
});